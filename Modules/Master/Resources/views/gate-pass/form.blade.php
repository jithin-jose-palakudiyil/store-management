    ...
    $supplier = \Modules\Master\Entities\PivotStoreItems::select('suppliers.*')
                            ...
                            ->where('pivot_store_items.id',$breakage->pivot_id) 
                            ->first();  
   
    ?>
    
    <?php if($shows && \Auth::guard(master_guard)->user()->role=='store'): ?>
    {!! Form::model($GatePass, ['method' => 'PATCH', 'route' => ['gate-pass.update',$GatePass->id,'breakage='.$breakage->id],'class'=>'form-valide','id'=>'_formUpdate','enctype'=>'multipart/form-data']) !!}    
    <div class="panel panel-flat">
