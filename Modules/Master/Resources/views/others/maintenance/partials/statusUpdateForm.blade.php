<?php if((count($maintenance->hasManyMaintenanceDates)>0)): ?>
<form id="statusUpdate" action="" name="statusUpdate">
    <input type="hidden" name="maintenance_id" id="maintenance_id" value="<?=$maintenance->id?>" >
    <div class="row">
        <div class="col-md-4">
            <div class="form-group">
                ...
                <select name="date" id="date" data-placeholder="select date" class="select " data-minimum-results-for-search="-1">
                    <option></option> 
                    <?php foreach ($maintenance->hasManyMaintenanceDates as $key => $value):
                        $status = null;
                        if($value->status==0):
                            $status ='initialized';
